...
export function buildDealTypeProductRecord({ productId, dealTypeId }) {
  return {
    product_id: normalizeForeignKey(productId, "product_id", { required: true }),
    deal_type_id: normalizeForeignKey(dealTypeId, "deal_type_id", {
      required: true,
    }),
  };
}

export function applyDealTypeProductUpdates(existingRecord, updates = {}) {
  ...
  if ("dealTypeId" in updates || "deal_type_id" in updates) {
    normalized.deal_type_id = normalizeForeignKey(
      updates.dealTypeId ?? updates.deal_type_id,
      "deal_type_id",
      { required: true }
    );
  }
